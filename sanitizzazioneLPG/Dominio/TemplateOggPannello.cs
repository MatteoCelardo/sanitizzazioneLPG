...
public class TemplateOggPannello
{
    ...
    private Type _modello;
    private string _etichetta;
    private StreamGeometry _icona;

    public TemplateOggPannello(Type mod, string icona, string etch)
    {
        _modello = mod;
        _etichetta = etch;

        // ricerca l'icona rappresentata dalla stringa "icona"
        Application.Current!.TryFindResource(icona, out var res);
        _icona = res as StreamGeometry ?? StreamGeometry.Parse(StreamGeometryNotFound);
        
    }

    public string Etichetta { get => _etichetta; }
    public Type Modello { get => _modello; }
    public StreamGeometry Icona { get => _icona; }
}
