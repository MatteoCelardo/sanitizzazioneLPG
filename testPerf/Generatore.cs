...
public class Generatore
{
    ...
    /// <summary>
    /// generatore di file d'input che crea diversi file con percentuali di etichette 
    ...
    public void GeneraFileUnNodo()
    {
        ...
        for(int i = 1; i <= _eticNodi.Count; i++)
        {
            ...
            nodo += "]\n}";
            File.WriteAllText("./fileInput/FileSingoloNodo_"+Math.Round((double)i/_eticNodi.Count * 100,2)+"-0.json",nodo);
        }
        ...
        for(int i = 1; i <= _propNodi.Count; i++)
        {
            ...
            nodo += "]\n}";
            File.WriteAllText("./fileInput/FileSingoloNodo_0-"+Math.Round((double)i/_propNodi.Count * 100,2)+".json",nodo);
        }
    ...
    private string GeneraSingoloNodo(double percEtic, double percProp)
    {
        ...
        double maxProp = _propNodi.Count * percProp / 100 >= 1 ? Math.Round((double)_propNodi.Count * percProp / 100,5) : 1;  
